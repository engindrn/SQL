...
(
id serial primary key  ,  --serial id yi auto increment gibi otomatik artırır
isim varchar(20),
yas int,
maas int,
email varchar(30)
...
select * from personel where id=8 --bütün bilgileri getirmek * dedik. tek id olsaydı id yazardık
...
select * from personel order by yas;      --**** natur siralama büyükten küçüğe asc
...
select * from personel order by yas desc; --*** büyükten küçüğe sıralar
...
select * from personel order by maas desc limit 3; ---****** sınırlandırma komutu

--21) yaşı 39 olmayan personeli sıralayınız
select * from personel where not yas=39
