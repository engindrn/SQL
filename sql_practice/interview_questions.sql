...
where id in(select id from isciler where isciler.id=personel6.id)
order by isim;

-- 2) Personel6 tablosunda kac tane kayit oldugunu gosteren query yazin
select count(id)  from isciler;
...
SELECT max(maas) from isciler      -- bu şekilde sadece en yüksek maas gelir
...
--subquery ile:
SELECT max(maas) AS enyuksek_ikinci_maas from isciler    -- as ile getirdiğimiz dataya head(başlık)ekledik
WHERE maas < (SELECT max(maas) from isciler)
...
select * from isciler 
order by maas desc         --maasları büyükten küçüğe sıraladık
offset 1 row             -- bir sutun atla dedik 
fetch next 1 row only     --fetch(gidip getirmek)sadece sonrakii bir sutunu

--sadece en yüksek alan ikinci kişinin iisim ve maas ını sunquery ile almak

select isim, maas from isciler
where maas=(select max(maas)from isciler
			where maas< (select max(maas)from isciler))
