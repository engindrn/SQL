...
--  LIKE ile 
 ...
 select * from kelimeler WHERE kelime ~~* '%at%' or kelime ~~* '%ot%'; -- BUYUK HARF KUCUK HARF FARKETMEZ  
-- REGEX ile 
select * from kelimeler WHERE kelime ~* 'ot' or kelime ~* 'at'; -- REGEX TE ---> BUNU ~ 1 KERE KULLANIYORUZ 
...
-- Regex te ikinci karakter icin [] parantez kullanilir misal  ho ve hi isteniyor [oi] h den sonra 2.harf o veya i olsun 
...
--SIMILAR TO ile
 select * from kelimeler where kelime similar to 'h[a-z,A-Z,0-9]t';
  --similar kullandigimiz icin [a-z,A-Z,0-9] bu h ile t arasindaki buyuk kucuk a- z, A-Z ve 0-9 
 --ornek h9t vardi eger biz   0-9 yazmasaydik    run yapinca h9t gorunmeyecekti 
--like ile
...
select * from kelimeler where kelime ilike 's%' or kelime ilike 'a%' or kelime ilike 'y%';
...
select * from kelimeler where kelime ilike '%m' or kelime ilike '%a' or kelime ilike '%f';
...
SELECT kelime FROM kelimeler WHERE kelime ~ '^[b-s](.*)l$';
select kelime from kelimeler where kelime similar to '[b-s]%l%'
