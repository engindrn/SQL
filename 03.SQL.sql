...
INSERT INTO calisanlar VALUES('10010', Mehmet Yılmaz, 5000, '2018-04-14'); --Unique olmalı yukarda tanımladık
...
INSERT INTO calisanlar VALUES( '10002', 'ayse Yılmaz' ,12000, '2018-04-14'); --PRIMARY KEY daha önce kullanıldı
...
adres_id char(5),       --primary gibi burdan yapamayız altta olyşturduk
sokak varchar(20),	
cadde varchar(30),					 
sehir varchar(20),					 
CONSTRAINT id_fk FOREIGN KEY (adres_id) REFERENCES calisanlar(id)   --adres_id yi calışanlar ıd si ile foreign ypt
--FOREIGN KEY (adres_id) REFERENCES calisanlar(id)	 bu şekildede oluşturulur ismi kendi verir
);
...
INSERT INTO adresler VALUES(NULL,'Ağa Sok', '30.Cad.','Antep'); --null atadık foreign key
...
delete from calisanlar where id='10002' --patent
delete from adresler where adres_id='10002' --child - önce childan silinmeli sonra parent

--ON DELETE CASCADE--
--Her defesında önce childaki verileri silmek yerine
--ON DELETE CASCADE özelliğini aktif hala getiririz
--bunun için Foreign key olan satırın en sonuna  ON DELETE CASCADE kommutunu yamak yeterli
...
--ON DELETE CASCADE kullandığımız için parent tan direk silebildik
--parent table dan sildiğimiz için child tebla dan da silinmiş oldu
delete table talebeler; -- hepsini siler

Drop table talebeler CASCADE; --parent tabloyu kaldırmak istersek CASCADE komutunu kullanırız

--talebeler tablosundaki isim sutununa NOT NULL kısıtlaması ekleyiniz ve veri type ini varchar(30) olr değiştiriniz
alter table talebeler 
alter column isim TYPE VARCHAR(30), --TYPE İLE DATA TÜRÜNÜ DEĞİŞTİRİP
alter column isim SET NOT NULL;     --SET İLE DE NOT NULL YAPTIK BOŞ BIRAKILAMAZ 

--TALEBELER TABLOSUNDAKİ yazili_notu sutununa 60 dan büyük rakam girilebilsin
alter table talebeler
ADD CONSTRAINT sinir CHECK (yazili_notu>60);
--CHECK kısıtlamsı ile tabloda istediğimiz sutunu sinirlandirabiliriz
...
Create table ogrenci_adres AS SELECT id, adres from ogrenciler;
...
alter table ogrenciler
ADD CONSTRAINT pk_id primary key(id); -- ismi biz belirledik
